...
export function useToolCards() {
		...
		const handleMouseMove = (e: MouseEvent) => {
			...
			if (header && tool) {
				...
				if (
					...
					if (
						selectedCard === null ||
						store.canvasStore.currentTool === 'select'
					) {
						document.body.style.cursor = 'default';
					} else {
						switch (selectedCard) {
	...
	}, [selectedCard, store.canvasStore.currentTool]);

	useEffect(() => {
		if (store.canvasStore.currentTool === 'select') {
			setSelectedCard(null);
		}
	}, [store.canvasStore.currentTool]);
